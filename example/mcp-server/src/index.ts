...
function getProjectRoot(): string {
  ...
  if (fs.existsSync(path.join(fromDist, "src", "api", "server.ts"))) return fromDist;
  const cwd = process.cwd();
  if (fs.existsSync(path.join(cwd, "src", "api", "server.ts"))) return cwd;
  return fromDist;
}
...
const TOOLS: Tool[] = [
  {
    ...
    description:
      "获取 DMS 维修移动端项目的架构说明（技术栈、路径别名、api/store/router/views/components 约定、检查清单与术语）。适合在增删改功能、重构或回答项目组织方式时使用。",
    inputSchema: {
      type: "object",
      properties: {},
    },
  },
  {
    ...
    description:
      "列出 src/api 下的业务模块与 server 中的接口前缀常量，便于查找或新增接口时选用正确前缀。",
    inputSchema: {
      type: "object",
      properties: {},
    },
  },
  {
    name: "list_routes",
    description:
      "列出路由表（path、name、meta.title 等），便于查找页面或新增路由时参考。",
    inputSchema: {
      type: "object",
      properties: {},
    },
  },
  {
    name: "list_stores",
    description:
      "列出 Pinia store 文件列表，便于查找状态模块或新增 store 时参考。",
    inputSchema: {
      type: "object",
      properties: {},
    },
  },
  {
    ...
    description:
      "读取项目内指定相对路径文件内容（相对项目根，如 src/api/server.ts）。用于在需要查看具体代码时由 AI 按路径读取。",
    inputSchema: {
      type: "object",
      properties: {
        relativePath: {
...
async function handleGetProjectArchitecture(): Promise<string> {
...
async function handleListApiModules(): Promise<string> {
  const serverContent = readFileSafe("src/api/server.ts");
  const entries = listDirSafe("src/api");
  const dirs: string[] = [];
  for (const name of entries) {
    if (name.startsWith(".") || name === "server.ts") continue;
    try {
      const full = path.join(PROJECT_ROOT, "src/api", name);
      if (fs.statSync(full).isDirectory()) dirs.push(name);
    } catch {
      /* skip */
    }
  }
...
async function handleListRoutes(): Promise<string> {
  const content = readFileSafe("src/router/index.ts");
  const routeMatches = content.matchAll(
    /path:\s*['"`]([^'"`]+)['"`]\s*,\s*name:\s*['"`]?(\w+)['"`]?[\s\S]*?title:\s*['"`]([^'"`]*)['"`]/g
  );
  const routes: { path: string; name: string; title: string }[] = [];
...
async function handleListStores(): Promise<string> {
  const files = listDirSafe("src/store").filter(
...
async function handleReadProjectFile(args: { relativePath?: string }): Promise<string> {
    ...
    try {
      ...
      switch (name) {
        ...
        case "read_project_file":
          result = await handleReadProjectFile(
            argsObj as { relativePath?: string }
          );
          break;
        default:
